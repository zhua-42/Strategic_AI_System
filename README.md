...
- **报告导出**：一键导出 Word 深度研报（封面 + 全部图表 + 证据链 + 正文 + 缺口说明 + 免责声明，Deloitte 绿/深蓝配色）与 **PPT 演示版研报**（16:9，图表页 + 要点页 + 证据链页 + 免责页）。
- **学习资料库**：内置 Forage 模拟（BCG / 摩根大通 / 花旗 / KPMG / PwC / Deloitte）与券商行研方法论（全流程 / 研报模板 / 财报分析 / 行研40问 / 深度报告写作）学习笔记，支持检索、下载与自上传入库。
...
industry_chain_data.py  # 产业链知识库（9+ 行业 × 5 环节：业务/龙头/成本/利润率/特点）
report_export.py        # 文档导出引擎（matplotlib 图表渲染 + Word + PPT，Deloitte 风格）
learning_data.py        # 学习资料库（Forage / 行研方法论入库与检索）
